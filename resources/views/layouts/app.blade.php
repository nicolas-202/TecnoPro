    ...
    <div class="flex flex-col min-h-screen">
        ...
        <header class="bg-blue-700 text-white p-4 flex justify-between items-center">
            ...
            <nav class="space-x-4">
                @auth
                    ...
                    @endphp
                    <a href="{{ route('user') }}" class="hover:underline">Usuario</a>
                    <a href="{{ route('cart') }}" class="hover:underline">Carrito de Compras</a>
        ...
        <nav class="bg-gray-500 p-4 shadow-md">
            <div class="flex space-x-6 overflow-x-auto">
                ...
                <a href="{{ route('catalog') }}" class="hover:text-blue-700 {{ request()->routeIs('catalog') ? 'text-blue-700 font-bold' : '' }}">Catálogo</a>
                ...
                @auth
                    @if(auth()->user()->empleado && auth()->user()->empleado->estadoEmpleado == 1)
                        <a href="{{ route('inventory') }}" class="hover:text-blue-700 {{ request()->routeIs('inventory') ? 'text-blue-700 font-bold' : '' }}">Inventario</a>
